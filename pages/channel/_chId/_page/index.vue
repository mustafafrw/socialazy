...
<script>
  ...
  export default {
    ...
    computed:{
      ...
      title()
      {
        if(this.$store.state.chData.length>0)
          return this.$store.state.chData[0].ch_name + " | Top liked youtube comments page "+this.$route.params.page;
        else
    ...
    async fetch ({ store, params }) {
      ...
      await store.dispatch('setchComments',[{id:params.chId,page:params.page}]);
    },
    head () {
      return {
        title: this.title,
    ...
    created() {
      if(this.$store.getters.getcurrentChPage===1)
          this.$store.commit("setcurrentChPage",this.$route.params.page);
    }
    /*mounted(){
